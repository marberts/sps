bp_replicates <- function(w, B = 1000, tau = 1, dist = NULL) {
  if (!is_positive_numeric(w) || any(w < 1)) {
    stop("'w' must be a positive and finite numeric vector which each element >= 1") 
  }
  ...
  res <- w * (a + tau) / tau
  if (any(res < 0)) warning("some replicate weights are < 0; try increasing 'tau'")
